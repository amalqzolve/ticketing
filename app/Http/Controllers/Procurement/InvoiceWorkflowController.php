...
class InvoiceWorkflowController extends Controller
{
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            $query->where('del_flag', 1)->where('flow_created_invoice', 1);
            $data = $query->get();
            $dataTble = Datatables::of($data)->addIndexColumn()->addColumn('action', function ($row) {
                return $row->id;
            })->rawColumns(['action']);
            $dataTble->editColumn('decription', function ($row) {
                $priority = InvoiceWorkflowModel::select('user_id', 'users.name')->leftjoin('users', 'invoiceworkflow.user_id', '=', 'users.id')->where('cat_id', '=', $row->id)->orderBy('priority', 'ASC')->get();
                $prList = '';
        ...
        return view('procurement.invoiceWorkflow.listing');
    }
    public function add(Request $request)
    {
        $query = MaterialCategoryModel::orderby('id', 'desc');
        $query->where('del_flag', 1)->where('flow_created_invoice', 0);
        $MaterialCategory = $query->get();
        $users = User::where('synthesis_user_flg', 1)->get();
        return view('procurement.invoiceWorkflow.add', compact('MaterialCategory', 'users'));
    }
        ...
        try {
            DB::transaction(function () use ($request) {
                ...
                for ($i = 0; $i < count($request->users); $i++) {
                    ...
                    $mr = InvoiceWorkflowModel::updateOrCreate(['id' => $postID], $data);
                }
                $MaterialCategory = MaterialCategoryModel::find($request->mrCat);
                if ($MaterialCategory)
                    $MaterialCategory->update(['flow_created_invoice' => 1]);
            });
            $out = array(
                'status' => 1,
                'msg' => 'Saved Success'
    ...
    public function editView(Request $request)
    {
        ...
        $MaterialCategory = $query->get();
        $users = User::where('synthesis_user_flg', 1)->get();
        $poWorkflow = InvoiceWorkflowModel::where('cat_id', '=', $request->id)->get();
        return view('procurement.invoiceWorkflow.edit', compact('MaterialCategory', 'users', 'poWorkflow'));
    }
    public function view(Request $request)
    {
        ...
        $users = User::where('synthesis_user_flg', 1)->get();
        $poWorkflow = InvoiceWorkflowModel::where('cat_id', '=', $request->id)->get();
        return view('procurement.invoiceWorkflow.view', compact('MaterialCategory', 'users', 'poWorkflow'));
    }
    ...
    public function update(Request $request)
    {
        try {
            DB::transaction(function () use ($request) {
                ...
                $useasr_id = Auth::user()->id;
                $branch = Session::get('branch');
                InvoiceWorkflowModel::where('cat_id', '=', $request->mrCat)->delete();
                for ($i = 0; $i < count($request->users); $i++) {
                    ...
                    $mr = InvoiceWorkflowModel::Create($data);
                }
            });
            $out = array(
                'status' => 1,
    ...
    public function clone(Request $request)
    {
        ...
        $users = User::where('synthesis_user_flg', 1)->get();
        $poWorkflow = InvoiceWorkflowModel::where('cat_id', '=', $request->id)->get();
        return view('procurement.invoiceWorkflow.clone', compact('MaterialCategory', 'users', 'poWorkflow'));
    }
    ...
    public function  delete(Request $request)
    {
        try {
            DB::transaction(function () use ($request) {
                ...
                if ($isUsed == 0) {
                    InvoiceWorkflowModel::where('cat_id', '=', $request->id)->delete();
