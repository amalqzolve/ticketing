...
use App\crm\salesDepartmentModel;
use DB;
...
class Salesdepartment extends Controller
{
    ...
    {
        return view('crm.Sales_Department.salesDepartment');
    }
    public function salesdepartment_index()
    {
        return view('crm.Sales_Department.salesDepartmenttrash');
    }
    ...
    {
        $postID = $request->id;
        salesDepartmentModel::where('id', $postID)->update(['del_flag' => 1]);
        return 'true';
    }
    ...
    {
        $request->validate(['title' => 'required', ], ['title.required' => 
            'title is required', ]);
        $user = auth()->user();
    ...
    {
        $postID = $request->id;
        salesDepartmentModel::where('id', $postID)->update(['del_flag' => 0]);
        return 'true';
    }
}
