...
class InvoiceController extends Controller
{
	...
	public function add($id)
	{
		...
		$invoiceAditionalAmount = InvoiceAditionalAmountModel::select('*')->where('car_rent_invoice_id', Crypt::decryptString($id))->get();
		return view('carRental.invoice.add', compact('branch', 'currencylist', 'vatlist',  'unitlist', 'termslist', 'salesmen', 'carInoutId_encrypted', 'car_in_out_id', 'carInAndOut', 'cars', 'totalAmount', 'totalAdditionalAmount'));
	}
	...
	public function edit(Request $request, $id)
	{
		...
		$carInAndOut =	CarInAndOut::select(
			...
			->first();
		$cars =  CarModel::select('id', 'registration_number', 'car_name')->where('branch', $branch)->get();
		$invoice = InvoiceModel::where('id', Crypt::decryptString($id))->first();
		if (isset($invoice->car_in_out_id)) {
			$carInoutId_encrypted = Crypt::encryptString($invoice->car_in_out_id);
			$car_in_out_id = $invoice->car_in_out_id;
		...
		return view('carRental.invoice.edit', compact('branch', 'currencylist', 'vatlist',  'unitlist', 'termslist', 'salesmen', 'carInoutId_encrypted', 'car_in_out_id', 'carInAndOut', 'invoice', 'invoiceAditionalAmount', 'cars', 'totalAmount', 'totalAdditionalAmount'));
	}
		...
		if ($request->ajax()) {
			...
			DB::transaction(function () use ($request) {
				...
				$data =  array(
					...
					'car_rent_performa_invoice_id' => $request->car_rent_performa_invoice_id,
					...
					'total_amount' => $request->total_amount, //inv amount
				...
				InvoiceAditionalAmountModel::where('car_rent_invoice_id', $invoiceModel->id)->delete();
				if (isset($request->additional_remarks)) {
					foreach ($request->additional_remarks as $key => $value) {
						...
						CarInAndOutAditionalAmount::where('id', $request->additional_cost_id[$key])->update(array('status' => 1));
					}
				}
				...
				if (($postID == '') || ($postID == null)) {
					...
					if ($ifFind) {
						...
						if ($request->additional_amount != '')
							$ifFind->increment('total_additional_amount_invoiced', $request->additional_amount);
					}
					$accArray = array(
						'car_in_out_id' => $request->car_in_out_id,
						'car_rent_payment_id' => NULL,
						'car_rent_invoice_id' => $invoiceModel->id,
						'type' => 'DEBIT',
						'transcation_date' => Carbon::now()->format('Y-m-d'),
						'trans_type' => 'INVOICE',
						'notes' => $request->description,
						'debit_amount' => $request->grandtotalamount,
						'credit_amount' => 0,
						'branch' => $branch,
					);
					...
					$ifFind =  CarInAndOut::find($request->car_in_out_id);
					if ($ifFind) {
						$ifFind->decrement('total_amount_invoiced', $oldAmount);
						$ifFind->increment('total_amount_invoiced', $request->amount);
						if ($oldAmountAdditional != '')
							$ifFind->decrement('total_additional_amount_invoiced', $oldAmountAdditional);
						if ($request->additional_amount != '')
							$ifFind->increment('total_additional_amount_invoiced', $request->additional_amount);
					}
					$accArray = array('debit_amount' =>  $request->grandtotalamount);
					StatementOfAccountsModel::where('car_rent_invoice_id', $postID)->update($accArray);
				}
			});
			$out = array(
				'status' => 1,
	...
	public function convertToReceipt(Request $request, $id)
	{
		$branch = Session::get('branch');
		$currencylist = DB::table('qpurchase_currency')->select('id', 'currency_name', 'currency_default', 'value')->where('branch', $branch)->where('del_flag', 1)->get();
		$unitlist = DB::table('qinventory_product_unit')->select('id', 'unit_name')->where('branch', $branch)->where('del_flag', 1)->get();
		...
		return view('carRental.invoice.convertToReceipt', compact('branch', 'currencylist', 'vatlist',  'unitlist', 'termslist', 'salesmen', 'carInoutId_encrypted', 'car_in_out_id', 'carInAndOut', 'invoice', 'cars'));
	}
	...
	public function delete(Request $request)
	{
		if ($request->ajax()) {
			try {
				DB::transaction(function () use ($request) {
					$adw = InvoiceModel::find($request->id);
					$ifFind =  CarInAndOut::find($adw->car_in_out_id);
					if ($ifFind) {
						$ifFind->decrement('total_amount_invoiced', $adw->amount);
					}
					$adw->delete();
				});
				$out = array(
	...
	public function Pdf(Request $request, $id)
	{
		...
		$salesinvoice   = InvoiceModel::select('*')->where('id', $id)->first();
		...
		// }
		$carInAndOut =	CarInAndOut::select('car_in_out.id', 'car_in_out.car_id', 'car_in_out.customer_id', DB::raw("DATE_FORMAT(car_in_out.isue_date, '%d-%m-%Y') as isue_date"), DB::raw("DATE_FORMAT(car_in_out.exp_date, '%d-%m-%Y') as exp_date"), DB::raw("DATE_FORMAT(car_in_out.trip_start_date, '%d-%m-%Y') as trip_start_date"), DB::raw("DATE_FORMAT(car_in_out.trip_end_date, '%d-%m-%Y') as trip_end_date"), 'trip_start_odometer', 'trip_end_odometer', 'rental_type', 'rate', 'limit', 'aditional_amount', 'renter_iqama', DB::raw("DATE_FORMAT(car_in_out.iqama_issue_date, '%d-%m-%Y') as iqama_issue_date"), DB::raw("DATE_FORMAT(car_in_out.iqama_exp_date, '%d-%m-%Y') as iqama_exp_date"), 'renter_licence_number', DB::raw("DATE_FORMAT(car_in_out.renter_licence_issue_date, '%d-%m-%Y') as renter_licence_issue_date"), DB::raw("DATE_FORMAT(car_in_out.renter_licence_exp_date, '%d-%m-%Y') as renter_licence_exp_date"), 'ins_id', 'ins_type', 'ins_amount', DB::raw("DATE_FORMAT(car_in_out.ins_start_date, '%d-%m-%Y') as ins_start_date"), DB::raw("DATE_FORMAT(car_in_out.ins_end_date, '%d-%m-%Y') as ins_end_date"))->where('car_in_out.id', $salesinvoice->car_in_out_id)->first();
		...
		if (isset($carInAndOut->id))
			...
			$customers = array();
		$invoiceAditionalAmount = InvoiceAditionalAmountModel::select('*')->where('car_rent_invoice_id', $id)->get();
		...
		if (Session::get('preview') == 'preview1') {
			$pdf = PDF::loadView('carRental.invoice.preview1', compact('branch', 'branchsettings', 'currencylist', 'termslist', 'customers', 'salesmen', 'salesinvoice', 'vatlist', 'bname', 'companysettings',  'brname', 'mrname', 'words', 'carInAndOut', 'invoiceAditionalAmount'));
		} elseif (Session::get('preview') == 'preview2') {

			$pdf = PDF::loadView('carRental.invoice.preview2', compact('branch', 'branchsettings', 'currencylist', 'termslist', 'customers', 'salesmen', 'salesinvoice', 'vatlist', 'bname', 'companysettings',  'brname', 'mrname', 'words', 'carInAndOut', 'invoiceAditionalAmount'));
		} elseif (Session::get('preview') == 'preview3') {
			$pdf = PDF::loadView('carRental.invoice.preview3', compact('branch', 'branchsettings', 'currencylist', 'termslist', 'customers', 'salesmen', 'salesinvoice', 'vatlist', 'bname', 'companysettings',  'brname', 'mrname', 'words', 'carInAndOut', 'invoiceAditionalAmount'));
		} elseif (Session::get('preview') == 'preview4') {
			if ($quote_status == "Approved") {
				$pdf = PDF::loadView('carRental.invoice.preview4', compact('branch', 'branchsettings', 'currencylist', 'termslist', 'customers', 'salesmen', 'salesinvoice', 'vatlist', 'bname', 'companysettings',  'words', 'carInAndOut', 'invoiceAditionalAmount'));
			} else {
				$pdf = PDF::loadView('carRental.invoice.preview4', compact('branch', 'branchsettings', 'currencylist', 'termslist', 'customers', 'salesmen', 'salesinvoice', 'vatlist', 'bname', 'companysettings',  'words', 'carInAndOut', 'invoiceAditionalAmount'), [], [
			...
			$pdf = PDF::loadView('carRental.invoice.preview4', compact('branch', 'branchsettings', 'currencylist', 'termslist', 'customers', 'salesmen', 'salesinvoice', 'vatlist', 'bname', 'companysettings', 'plabels', 'itemdetails', 'brname', 'mrname', 'words'));
		}
