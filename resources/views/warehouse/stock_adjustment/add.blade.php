...
<div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
	...
	?>		
								...
								<input type="text" class="form-control kt_datetimepickerr" name="adjustment_date" id="adjustment_date" value="{{date('d-m-Y')}}">               
										</div>
										</div>
										</div>
									...
									<div class="col-lg-6">
										...
										<div class="col-md-8 input-group-sm">
									   <select class="form-control kt-selectpicker" id="adjustedby">
            ...
            @endforeach
									...
									<div class="col-lg-12">
                                    <div class="form-group row pr-md-3 pl-md-0">
                                    <div class="col-md-2">
                                    <label>Reason</label>
                                    </div>  
                          ...
                          <table class="table table-striped table-bordered table-hover" id="product_table"  style="table-layout:fixed; width:100%">
                                    ...
                                    <th style="background-color:  #3f4aa0;
                                       ...
                                       <th style="background-color:  #3f4aa0;
                                       color: white; white-space: nowrap;     padding: 2px 7px !important;">Stock +/-</th>
                                       

                                    <th style="background-color:  #3f4aa0;
                                       color: white; white-space: nowrap;     padding: 2px 7px !important;"> Quantity</th>
												...
												<div class="kt-form__actions">
													<div class="row">
														...
														<div class="col-lg-6 kt-align-right">
															<button type="submit" name="stock_adjust_submit" id="stock_adjust_submit" class="btn btn-primary float-right ">
  ...
  // alert(product_name);
      ...
      success: function(data) { 
         ...
         product += '<tr>\
                 ...
                 <div class="input-group input-group-sm">\
                 <input type="text" class="form-control available_stock" name="available_stock[]" id="available_stock'+rowcount+'"  data-id="'+rowcount+'" value="'+value.available_stock+'" readonly>\
                 </div>\
                 </td>\<td>\
                 <div>\
                 <select class="form-control form-control-sm single-select type kt-selectpicker"  data-id="'+rowcount+'" name="atype[]" id="type'+rowcount+'">\
              ...
              <option value="2">-</option>\
                 ...
                 <div class="input-group input-group-sm">\
                 <input type="text" class="form-control stock_adjust_quantity" name="stock_adjust_quantity[]"  data-id="'+rowcount+'" id="stock_adjust_quantity'+rowcount+'" placeholder="+/- 10">\
                 </div>\
                 </td>\
...
$('body').on('change', '.stock_adjust_quantity', function() {
	 var id = $(this).attr('data-id');
	 
var stock_adjust_quantity = $('#stock_adjust_quantity'+id+'').val();

var type = $('#type'+id+'').val();

var available_stock = $('#available_stock'+id+'').val();

if(stock_adjust_quantity<1){
    toastr.error('Add Stock Adjustment Quantity');
}



if(type=='-'){
if (parseInt(stock_adjust_quantity) > parseInt(available_stock))

    {
         toastr.error('Stock Adjustment Quantity Must not less Zero');
        $(this).val("");
    } else {
        // do something
    }
...
$(document).on('click', '#stock_adjust_submit', function(e) {
    e.preventDefault();
 ...
 if (adjustment_date == "") {
    ...
    $('#adjustment_date').removeClass('is-invalid');
    }

adjustedby      = $('#adjustedby').val();
 if (adjustedby == "") {
            $('#adjustedby').next().find('.select2-selection').addClass('select-dropdown-error');
            toastr.warning("Please Add Any Salesman!");
                      return false;
        } else {
            $('#adjustedby').next().find('.select2-selection').removeClass('select-dropdown-error');
        }
...
/*reason      = $('#reason').val();
 if (reason == "") {
            $('#reason').next().find('.select2-selection').addClass('select-dropdown-error');
            toastr.warning("Please Add Any Reason!");
                      return false;
        } else {
            $('#reason').next().find('.select2-selection').removeClass('select-dropdown-error');
        }
*/
 ...
 var available_stock = [];
        ...
        .each(function(input) {
         ...
         var stock_adjust_quantity = [];

        $("input[name^='stock_adjust_quantity[]']")
        .each(function(input) {
            stock_adjust_quantity.push($(this).val());
        });
...
alert(atype);
    ...
    $.ajax({
        ...
        stock_adjust_quantity : stock_adjust_quantity,
        atype : atype,
        ...
        success: function(data) {
             ...
             $('#stock_adjust_submit').removeClass('kt-spinner');
             $('#stock_adjust_submit').prop("disabled", false);
             location.reload();
              window.location.href = "stock_adjust_list";
             toastr.success('Stock Adjustment '+sucess_msg+' successfuly');
