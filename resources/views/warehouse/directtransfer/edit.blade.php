...
<div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
		...
		<div class="kt-portlet__head kt-portlet__head--lg">
			<div class="kt-portlet__head-label">
				...
				<h3 class="kt-portlet__head-title">
					 {{ __('warehouse.Direct Stock Transfer') }}
				</h3>
			</div>
	...
	<?php
	$date = date('d-m-Y');
	foreach($direct as $directs)
	{
		$id = $directs->id;
		$trdate = $directs->transfer_date;
		$requested_by = $directs->requested_by;
		$notes = $directs->notes;
		$prepared_by = $directs->prepared_by;
		$approved_by = $directs->approved_by;
		$transfer_warehouse = $directs->transfer_warehouse;
	}
	?>		
								...
								<input type="text" class="form-control kt_datetimepickerr" name="out_date" id="out_date" value="{{$trdate}}">               
										</div>
										</div>
										</div>
            ...
            <option value="{{$data->id}}"@if($prepared_by == $data->id) {{"selected"}}@endif>{{$data->name}}</option>
            @endforeach
									   </select>
										...
										<div class="col-md-8 input-group-sm">
									   	...
									   	<option value="">select</option>
			@foreach($salesmen as $data)
            <option value="{{$data->id}}" @if($approved_by == $data->id){{"selected"}}@endif>{{$data->name}}</option>
            @endforeach
									   </select>
            ...
            <option value="{{$data->id}}" @if($transfer_warehouse == $data->id){{"selected"}}@endif>{{$data->warehouse_name}}</option>
            @endforeach
									   </select>
									   ...
									   <div class="col-lg-6">
										<div class="form-group  row pr-md-3">
										<div class="col-md-4">
										<label>@lang('warehouse.Requested By')</label>
										</div>  
										<div class="col-md-8 input-group-sm">
									   
								<input type="text" class="form-control" name="requested_by" id="requested_by" value="{{$requested_by}}">               
										</div>
										</div>
										</div>
                          ...
                          <table class="table table-striped table-bordered table-hover" id="product_table"  style="table-layout:fixed; width:100%">
                                    ...
                                    <th style="background-color:  #3f4aa0;
                                       color: white; white-space: nowrap;     padding: 2px 7px !important;">@lang('warehouse.Existing Quantity')</th>
                                    <th style="background-color:  #3f4aa0;
                                       color: white; white-space: nowrap;     padding: 2px 7px !important;">@lang('warehouse.Stock Transfer Quantity')</th>
                                    ...
                                    <th style="background-color:  #3f4aa0;
                                       color: white; white-space: nowrap;     padding: 2px 7px !important; " width="60px">@lang('app.Action')</th>
												...
												<tbody>
													@foreach($directproducts as $key=>$directproducts)
												<tr>
                 <td>{{$key+1}}</td><td>
                 <div class="input-group input-group-sm">
                 <input type="text" class="form-control single-select productname kt-selectpicker" name="productname[]" id="productname" data-id="{{$key+1}}" value="{{$directproducts->product_name}}" readonly>
                 <div>
                 <input type="hidden" class="form-control single-select item_details_id" name="item_details_id[]" id="item_details_id" data-id="{{$key+1}}" value="{{$directproducts->productname}}">
                 </td><td>
                 <div class="input-group input-group-sm">
                 <input type="text" class="form-control available_stock" name="available_stock[]" id="available_stock{{$key+1}}"  data-id="{{$key+1}}" value="{{$directproducts->existing_quantity}}" readonly>
                 </div>
                 </td>
                 <td>
                 <div class="input-group input-group-sm">
                 <input type="text" class="form-control stock_out_quantity" name="stock_out_quantity[]"  data-id="{{$key+1}}" id="stock_out_quantity{{$key+1}}" value="{{$directproducts->stock_out_quantity}}">
                 </div>
                 </td>
												...
												<div class="kt-form__actions">
													<div class="row">
														...
														<div class="col-lg-6 kt-align-right">
															<button type="submit" name="directstocktransferupdate" id="directstocktransferupdate" class="btn btn-primary float-right ">
...
$(document).on('click', '#directstocktransferupdate', function(e) {
    e.preventDefault();
 ...
 var available_stock = [];
    ...
    $.ajax({
        ...
        success: function(data) {
             ...
             $('#directstocktransferupdate').removeClass('kt-spinner');
             $('#directstocktransferupdate').prop("disabled", false);
             location.reload();
              window.location.href = "directstocktransfer";
             toastr.success('Stock Out '+sucess_msg+' successfuly');
