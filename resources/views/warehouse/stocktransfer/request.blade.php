...
<div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
	...
	?>
								...
								<input type="text" class="form-control kt_datetimepickerr" name="request_date" id="request_date" value="{{date('d-m-Y')}}">
										</div>
										</div>
										</div>
									...
									<div class="col-lg-6">
										...
										<div class="col-md-8 input-group-sm">
									   <select class="form-control kt-selectpicker" id="requestedby">
            ...
            @endforeach
                          ...
                          <table class="table table-striped table-bordered table-hover" id="product_table"  style="table-layout:fixed; width:100%">
                                    ...
                                    <th style="background-color:  #3f4aa0;
                                       color: white; white-space: nowrap;     padding: 2px 7px !important;">Transfer Request Quantity</th>
												...
												<div class="kt-form__actions">
													<div class="row">
														...
														<div class="col-lg-6 kt-align-right">
															<button type="submit" name="stock_transfer_request_submit" id="stock_transfer_request_submit" class="btn btn-primary float-right ">
...
$(document).on('click', '#stock_transfer_request_submit', function(e) {
    e.preventDefault();
...
requestedby      = $('#requestedby').val();
 if (requestedby == "") {
            $('#requestedby').next().find('.select2-selection').addClass('select-dropdown-error');
            toastr.warning("Please Add Any Salesman!");
                      return false;
        } else {
            $('#requestedby').next().find('.select2-selection').removeClass('select-dropdown-error');
        }
...
warehouse      = $('#warehouse').val();
 ...
 var available_stock = [];
    ...
    $.ajax({
        type: "POST",
        url: "stocktransferrequestsubmit",
        dataType: "text",
        data: {
            _token: $('#token').val(),
        request_date            : $('#request_date').val(),
        ...
        success: function(data) {
             ...
             $('#stock_transfer_request_submit').removeClass('kt-spinner');
             $('#stock_transfer_request_submit').prop("disabled", false);
             location.reload();
              window.location.href = "stock_transfer_request_list";
             toastr.success('Stock Transfer Request '+sucess_msg+' successfuly');
