							...
							<div class="kt-grid__item kt-grid__item--fluid kt-wizard-v1__wrapper">
								<form class="kt-form" id="kt_form">
									<div class="kt-wizard-v1__content p-0" data-ktwizard-type="step-content">
										...
										<div class="kt-form__section kt-form__section--first w3-animate-right" id="animation">
											<div class="kt-wizard-v1__form">
												<div class="separator separator-dashed separator-border-2 mb-5"></div>@php foreach ($data as $key => $value) {$noinv = $value->no_of_invoices; $cpeinv= $value->credit_period_of_each_invoices; $cptinc= $value->credit_period_of_total_invoices; $tamnt = $value->total_amount; $pterms= $value->payment_terms; $description = $value->description; $pterms =isset($pterms)? $pterms :'';
															...
															<table class="table table-striped table-hover" id="modeofpaymenttable">
																<thead style=" background-color: #306584; color: white;">
																	<tr>
																		...
																		<th class="text-center" style="background-color:  #3f4aa0;     color: white; white-space: nowrap;     padding: 2px 7px !important;">@lang('customer.Document Name')</th>
																		<th class="text-center" style="background-color:  #3f4aa0;     color: white; white-space: nowrap;     padding: 2px 7px !important;">@lang('customer.Expiry Date')</th>
																		<th class="text-center" style="background-color:  #3f4aa0;     color: white; white-space: nowrap;     padding: 2px 7px !important;">@lang('customer.Reminder Days')</th>
												 ...
												 width: 30px;">
															...
															<i class="fa fa-plus" style="color: white;"></i>
																...
																<tr>
																	...
																	<td style="padding: 0;">
																		<input class="form-control single-select documentname" name="documentname[]" id="documentname{{$key+1}}" data-id={{$key+1}} placeholder="@lang('customer.Document Name')" value="{{$docs->docname}}">
																	</td>
																	<td style="padding: 0;">
																		<div class="input-group input-group-sm">
																			<input type="text" class="form-control kt_datetimepickerr expirydate" placeholder="@lang('customer.Expiry Date')" id="expirydate{{$key+1}}" name="expirydate[]" data-id={{$key+1}} value="{{$docs->expdate1}}">
																		</div>
																	</td>
																	<td style="padding: 0;">
																		<div class="input-group input-group-sm">
																			<input type="text" class="form-control days" name="days[]" id="days{{$key+1}}" data-id={{$key+1}} placeholder="@lang('customer.Reminder Days')" value="{{$docs->days}}">
																		</div>
																	<td style="padding: 0;">
												...
												<div class="row">
													<div class="col-lg-6">
														<div class="form-group row pr-md-3">
															<div class="col-md-4">
																<label>{{ __('customer.Number Of Invoice') }}</label>
															</div>
															<div class="col-md-8">
																<div class="input-group input-group-sm">
																	<input type="text" class="form-control no_of_invoices" name="no_of_invoices" id="no_of_invoices" placeholder="{{ __('customer.Number Of Invoice') }}" value="{{isset($noinv)? $noinv :''}}">
																</div>
															</div>
														</div>
													</div>
													<div class="col-lg-6">
														<div class="form-group row pl-md-3">
															...
															<div class="col-md-8 input-group input-group-sm">
																<div class="input-group input-group-sm">
																	<input type="number" class="form-control Amount" placeholder="{{ __('customer.Total Amount') }}" id="total_amount" name="total_amount" value="{{isset($tamnt)? $tamnt :''}}">
																</div>
															</div>
														</div>
													</div>
													<div class="col-lg-6">
														<div class="form-group row pr-md-3">
															<div class="col-md-4">
																<label>{{ __('customer.Credit Period Of Each Invoice') }}</label>
															</div>
															<div class="col-md-8">
																<div class="input-group input-group-sm">
																	<input type="text" class="form-control credit_period_each_invoice" name="credit_period_each_invoice" id="credit_period_each_invoice" placeholder="{{ __('customer.Credit Period Of Each Invoice') }}" value="{{isset($cpeinv)? $cpeinv :''}}">
																</div>
															</div>
														</div>
													</div>
									...
									<div class="kt-wizard-v1__content" data-ktwizard-type="step-content">
										...
										<div class="kt-form__section kt-form__section--first w3-animate-right" id="animation">
											<div class="kt-wizard-v1__form">
												...
												<div class="row">
													<div class="col-lg-12">
														<label>{{ __('customer.Description') }}</label>
														<textarea id="description1" name="description1" class="form-control" style="width:
												100%;">{{isset($description)? $description :''}}</textarea>
													</div>
												</div>
											</div>
										</div>
									...
									<div class="kt-form__actions">
										...
										<button class="btn btn-brand btn-elevate btn-icon-sm" data-ktwizard-type="action-submit" id="customer_documents_submit"><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-check-circle icon-16">
...
<script>
	...
	$(".addmorepayments").click(function() {
		...
		payment += '<tr>\
					  ...
					  <td style="padding: 0;">\
					  <input class="form-control single-select documentname" name="documentname[]" id="documentname' + rowcount + '" data-id=' + rowcount + ' placeholder="@lang("customer.Document Name")">\
					  </td>\
					  <td style="padding: 0;">\
					  <div class="input-group input-group-sm">\
					 <input type="text" class="form-control kt_datetimepickerr expirydate" placeholder="@lang("customer.Expiry Date")" id="expirydate' + rowcount + '" name="expirydate[]" data-id=' + rowcount + '>\
					  </div>\
					  </td>\
					  <td style="padding: 0;">\
					  <div class="input-group input-group-sm">\
					  <input type="text" class="form-control days" name="days[]" id="days' + rowcount + '"  data-id=' + rowcount + ' placeholder="@lang("customer.Reminder Days")">\
					  </div>\
						<td style="padding: 0;">\
					  <div class="kt-demo-icon__preview remove">\
					  <i class="fa fa-trash" id="remove_row" style="color: red;padding-left: 30%;"></i>\
	...
	allowedFileTypes: ["image/*"]
		...
		onBeforeUpload: (files) => {
			...
			Object.keys(files).forEach(fileID => {
				...
				fileData.push('custdocumentInfoData/' + files[fileID]
					.name)
	...
	uppy.use(Uppy.XHRUpload, {
		endpoint: 'crmcustdocumentFileUpload',
	...
	uppy.on('file-removed', (file, reason) => {
		...
		var file1name = 'custdocumentInfoData/' + file.name;
		var ofile = file.name;
	...
	$(document).on('click', '#customer_documents_submit', function(e) {
		e.preventDefault();
		...
		// var userss = $("select[name='user_id[]']").map(function(){return $(this).val();}).get();
		...
		$.ajax({
			...
			success: function(data) {
				...
				window.location.href = "crmcustomerdocuments";
	...
	$(document.body).on("keyup  change", ".days", function() {
		var $this = $(this);
		$this.val($this.val().replace(/[^\d.]/g, ''));
	});
	$('.kt_datetimepickerr').datepicker({
