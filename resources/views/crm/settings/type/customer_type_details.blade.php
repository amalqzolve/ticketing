...
<div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
	...
	<div class="kt-portlet kt-portlet--mobile">
		<div class="kt-portlet__head kt-portlet__head--lg">
			<div class="kt-portlet__head-label"> <span class="kt-portlet__head-icon">
				...
				<h3 class="kt-portlet__head-title">
					{{ __('app.Customer Type List') }}
				</h3>
			</div>
			<div class="kt-portlet__head-toolbar">
				<div class="kt-portlet__head-wrapper">
					<div class="kt-portlet__head-actions">
						@can('Customer Type Add')
						<button type="button" class="btn btn-brand btn-elevate btn-icon-sm" data-type="add" data-toggle="modal" data-target="#kt_modal_4_5"><i class="la la-plus"></i>{{ __('app.New Record') }}</button>@endcan
						<div class="dropdown dropdown-inline">
							...
							<div class="dropdown-menu dropdown-menu-right">
								<ul class="kt-nav">
									...
									<li class="kt-nav__item" id="customertypedetails_list_1_print"> <span href="#" class="kt-nav__link">
											<i class="kt-nav__link-icon la la-print"></i>
											<span class="kt-nav__link-text">@lang('app.Print')</span>
										</span>
									</li>
									<li class="kt-nav__item" id="customertypedetails_list_1_copy"> <span class="kt-nav__link">
											<i class="kt-nav__link-icon la la-copy"></i>
											<span class="kt-nav__link-text">@lang('app.Copy')</span>
										</span>
									</li>
									<li class="kt-nav__item" id="customertypedetails_list_1_csv">
										<a href="#" class="kt-nav__link"> <i class="kt-nav__link-icon la la-file-text-o"></i>
											<span class="kt-nav__link-text">@lang('app.CSV')</span>
										</a>
									</li>
									<li class="kt-nav__item" id="customertypedetails_list_1_pdf"> <span class="kt-nav__link">
						...
						<!-- <a href="{{url('/')}}/settingstypetrash" type="button" class="btn btn-secondary btn-hover-warning btn-icon-sm">@lang('app.Trash')
						</a> -->
					</div>
				</div>
			</div>
		...
		<div class="kt-portlet__body">
			<table class="table table-striped  table-hover table-checkable" id="customertypedetails_list_1">
				<thead>
					<tr>
						<th>{{ __('app.Sl. No') }}</th>
						<th>{{ __('customer.Customer Type') }}</th>
						<th>{{ __('app.Default') }}</th>
						<th>{{ __('customer.Colour') }}</th>
						<th>{{ __('app.Note') }}</th>
						<th>{{ __('app.System ID') }}</th>
	...
	<div class="modal-dialog modal-xl" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title" id="exampleModalLabel">{{ __('customer.Customer Type') }}</h5>
						...
						<div class="form-group row">
								...
								<div class="form-group row pr-md-3">
									...
									<div class="col-md-8   pl-md-2">
										<div class="input-group  input-group-sm">
											<input type="text" class="form-control" placeholder="{{ __('customer.Customer Type') }}" id="title" name="title" autocomplete="off">
										</div>
									</div>
								</div>
							</div>
							...
							<div class="col-lg-6">
								<div class="form-group row pr-md-1">
									...
									<div class="col-md-8 pl-4">
										<div class="input-group  input-group-sm">
											<select class="form-control typedefault single-select kt-selectpicker" id="typedefault" name="typedefault">
							...
							<div class="col-lg-6">
								<div class="form-group row pr-md-3">
									...
									<div class="col-md-8  pl-md-1 pr-md-0">
										<div class="input-group  input-group-sm">
			...
			<div class="modal-footer">
				...
				<button type="reset" class="btn btn-secondary float-right mr-2 closeBtn" onclick="closeModels()"><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-x icon-16">
						<line x1="18" y1="6" x2="6" y2="18"></line>
						<line x1="6" y1="6" x2="18" y2="18"></line>
					</svg> {{ __('app.Cancel') }}</button>
				<button id="Type_submit" class="btn btn-brand kt-spinner--right kt-spinner--sm kt-spinner--light"><svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-check-circle icon-16">
...
<script src="{{url('/')}}/resources/js/settings/type.js" type="text/javascript"></script>

<script type="text/javascript">
	var customertypedetails_table = $('#customertypedetails_list_1').DataTable({
		processing: true,
		serverSide: true,
		...
		ajax: {
			"url": 'settingscustomertypedetails',
			"type": "POST",
			"data": function(data) {
	...
	$("#customertypedetails_list_1_print").on("click", function() {
		customertypedetails_table.button('.buttons-print').trigger();
	});


	$("#customertypedetails_list_1_copy").on("click", function() {
		customertypedetails_table.button('.buttons-copy').trigger();
	});

	$("#customertypedetails_list_1_csv").on("click", function() {
		customertypedetails_table.button('.buttons-csv').trigger();
	});

	$("#customertypedetails_list_1_pdf").on("click", function() {
	...
	$(document).on('click', '#Type_submit', function(e) {
		...
		$.ajax({
			...
			data: {
				...
				color: $('#color').val(),
				branch: $('#branch').val(),
				typedefault: $('#typedefault').val()
			},
			success: function(data) {
				if (data == false) {
					$('#Type_submit').removeClass('kt-spinner');
					$('#Type_submit').prop("disabled", false);
					toastr.warning('Customer Type Name Is Already Exist');
		...
		$.ajax({
			...
			success: function(data) {
				$.each(data, function(key, value) {
					...
					$('#description').val(value.discription);
					$('#color').val(value.color);
					$(".typedefault").val(value.typedefault).trigger("change");
...
<script>
	...
	$('.settingscustomertypedetails').addClass('kt-menu__item--active');
</script>
