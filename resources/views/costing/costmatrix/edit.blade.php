...
<?php
foreach ($Costmatrix as $Costmatrixs) {
   $id = $Costmatrixs->id;
   $costmatrixname = $Costmatrixs->costmatrixname;
   $description = $Costmatrixs->description;
   $totalamount = $Costmatrixs->totalamount;
   $discount = $Costmatrixs->discount;
   $amountafterdiscount = $Costmatrixs->amountafterdiscount;
   $totalvatamount = $Costmatrixs->totalvatamount;
   $grandtotalamount = $Costmatrixs->grandtotalamount;
}


?>
<input type="hidden" name="costmatrixid" id="costmatrixid" value="{{$id}}">
            ...
            <div class="row" style="padding-bottom: 6px;">
                  ...
                  <div class="form-group row pr-md-3">
                     ...
                     <div class="col-md-8">
                        <div class="input-group input-group-sm">
                           <input type="text" class="form-control" name="costmatrixname" id="costmatrixname" placeholder="Cost Matrix Name" autocomplete="off" value="{{$costmatrixname}}">
                        </div>
                     </div>
                  </div>
               </div>
               ...
               <div class="col-lg-6">
                  <div class="form-group row pr-md-3">
                     ...
                     <div class="col-md-8">
                        <div class="input-group input-group-sm">
                           <input type="text" class="form-control" name="description" id="description" placeholder="{{ __('app.Note') }}" autocomplete="off" value="{{$description}}">
                        </div>
                     </div>
                  </div>
               </div>
            ...
            <div class="row" style="padding-bottom: 6px;">
               <div class=" pr-1 pl-1" style="width: 100%;">
                  <table class="table table-striped table-bordered table-hover" id="product_table" style="width:100%">
                     ...
                     <br>
                     <tbody>
                        @foreach($CostmatrixProduct as $key=>$CostmatrixProducts)
                        <tr>
                           <td style="text-align: center;">{{$key+1}}</td>
                           <td>
                              <input type="text" class="form-control head_name" name="head_name[]" id="head_name1{{$key+1}}" value="{{$CostmatrixProducts->product_name}}" style="height: 31px;">
                           </td>
                           <td><textarea class="form-control" id="product_description{{$key+1}}" name="product_description[]" rows="1" data-id="{{$key+1}}" style=" height: 30px !important;">{{$CostmatrixProducts->description}}</textarea></td>
                           <td>
                              <div>
                                 <select class="form-control form-control-sm single-select unit kt-selectpicker" data-id="{{$key+1}}" name="unit[]" id="unit{{$key+1}}">
                                    ...
                                    <option value="{{$data->id}}" <?php if ($CostmatrixProducts->unit == $data->id) echo "selected"; ?>>{{$data->unit_name}}</option>
                                    @endforeach
                                 </select>
                              </div>
                           </td>
                           <td>
                              <div class="input-group input-group-sm"> <input type="text" class="form-control quantity" data-id="{{$key+1}}" name="quantity[]" id="quantity{{$key+1}}" value="{{$CostmatrixProducts->quantity}}"> </div>
                           </td>
                           <td>
                              <div class="input-group input-group-sm"> <input type="text" class="form-control rate" name="rate[]" id="rate{{$key+1}}" data-id="{{$key+1}}" value="{{$CostmatrixProducts->rate}}"> </div>
                           </td>
                           <td>
                              <div class="input-group input-group-sm"> <input type="text" class="form-control amount" name="amount[]" data-id="{{$key+1}}" id="amount{{$key+1}}" readonly="" value="{{$CostmatrixProducts->amount}}"> </div>
                           </td>
         ...
         <div class="kt-form__actions">
            <div class="row">
               ...
               <div class="col-lg-6 kt-align-right">
                  <button id="costmatrixupdate" class="btn btn-primary float-right ">
                  ...
                  <button type="button" class="btn btn-secondary mr-2 backHome" onclick="goPrev()">
...
<script>
   ...
   $(document).on('click', '#costmatrixupdate', function(e) {
      e.preventDefault();
      var head_name = [];
      ...
      $.ajax({
         ...
         success: function(data) {
            if (data == 'false') {
               $('#costmatrixupdate').removeClass('kt-spinner');
               $('#costmatrixupdate').prop("disabled", false);
