							...
							<div class="col-lg-6">
								<div class="form-group  row pr-md-3">
									...
									<div class="col-md-8 input-group-sm">
										<input type="text" class="form-control kt_datetimepickerr" name="date" id="date" value="{{date('d-m-Y',strtotime($refund->date)) }}">
									</div>
								</div>
							</div>
							<div class="col-lg-6">
								<div class="form-group  row pr-md-3">
									...
									<div class="col-md-8 input-group-sm">
										<select class="form-control kt-selectpicker" id="rec_by" name="rec_by">
											...
											<option value="{{$man->id}}" {{($refund->rec_by==$man->id)?'selected':''}}>{{$man->name}} </option>
											@endforeach
										</select>
									</div>
								</div>
							...
							<div class="col-lg-12">
								<div class="form-group  row pr-md-3">
									...
									<div class="col-md-10 input-group-sm">
										<textarea class="form-control" name="notes" id="notes">{{$refund->notes}}</textarea>
									</div>
								</div>
							</div>
						</div>
						...
						<div class="row">
							...
							<div class="col-lg-6">
								<div class="form-group  row pr-md-3">
									...
									<div class="col-md-8 input-group-sm">
										<input type="text" class="form-control " value="{{$return->code.'/'.$return->br_id}}" readonly>
									</div>
								</div>
							</div>
							<div class="col-lg-6">
							...
							<div class="form-group  row pr-md-3">
								...
								<div class="col-md-9 input-group-sm">
									<select class="form-control single-select kt-selectpicker" id="terms_conditions" name="terms_conditions">
										...
										<option value="{{$data->id}}" {{($refund->terms_conditions==$data->id)?'selected':''}}>{{$data->term}}</option>
										@endforeach
									</select>
								</div>
							</div>
							...
							<div class="form-group  row pr-md-3">
								<div class="col-md-12 input-group-sm">
									<div class="kt-tinymce">
										<textarea id="kt-tinymce-4" name="kt-tinymce-4" class="tox-target">{!!$refund->tpreview!!}</textarea>
									</div>
								</div>
							</div>
						</div>
				...
				<div class="row mt-5">
					...
					<div class="col-lg-6">
						<div class="form-group  row pr-md-3">
							...
							<div class="col-md-8 input-group-sm">
								<input type="text" class="form-control" name="grandtotalamount" id="grandtotalamount" readonly value="{{$return->supplier_given_amt-$refund->addtotal}}" style="background-color: #ffffff00;  border: none;  text-align: right;  font-size: 1.5rem; font-weight: bold; color: #646c9a; padding-top: 0px;">
							</div>
						</div>
					</div>
					<div class="col-lg-6"></div>
					<div class="col-lg-6">
						<div class="form-group  row pr-md-3">
							...
							<div class="col-md-8 input-group-sm">
								<input type="text" class="form-control" name="pending_balance_amt" id="pending_balance_amt" readonly value="{{$return->grandtotalamount-($return->supplier_given_amt-$refund->addtotal)}}" style="background-color: #ffffff00;  border: none;  text-align: right;  font-size: 1.75rem; font-weight: bold; color: #646c9a; padding-top: 0px;">
							</div>
						</div>
					</div>
				</div>
			...
			<div class="col-lg-12">
				<div class="form-group row pl-md-3">
					<table class="table table-striped table-hover" id="modeofpaymenttable">
						...
						<tbody>
							@foreach($refundProducts as $key=>$refundProduct)
							<tr>
								...
								<td style="padding: 0;">
									<select class="form-control" id="debitaccount[]" name="debitaccount[]">
										...
										<option value="{{$ledger->id}}" {{($refundProduct->debitaccount==$ledger->id)?'selected':''}}>[{{$ledger->code}}] {{$ledger->name}} </option>
										@endforeach
									</select>
								</td>
								<td style="padding: 0;">
									<div class="input-group input-group-sm">
										<input type="text" class="form-control reference" name="reference[]" id="reference0" data-id="0" value="{{$refundProduct->reference}}">
									</div>
								</td>
								<td style="padding: 0;">
									<div class="input-group input-group-sm">
										<input type="text" class="form-control amount integerVal" name="amount[]" id="amount0" data-id="0" value="{{$refundProduct->amount}}">
									</div>
								</td>
								<td style="padding: 0;">
					...
					<div class="row col-lg-12">
						...
						<div class="col-lg-6">
							<div class="form-group  row pr-md-3">
								...
								<div class="col-md-8 input-group-sm">
									<input type="text" class="form-control" name="addtotal" id="addtotal" value="{{$refund->addtotal}}" readonly>
								</div>
							</div>
						</div>
					</div>
