   ...
   <div class="kt-portlet kt-portlet--mobile">
      <div class="kt-portlet__head kt-portlet__head--lg">
         <div class="kt-portlet__head-label">
            ...
            <h3 class="kt-portlet__head-title">Sales Invoice Settlement - Payment Vouchers
            </h3>
         </div>
         <div class="kt-portlet__head-toolbar">
            <div class="kt-portlet__head-wrapper">
               <div class="kt-portlet__head-actions">
                  <a type="button" class="btn btn-brand btn-elevate btn-icon-sm" href="{{url('qpurchase-bill-settlement-add')}}"><i class="la la-plus"></i>{{ __('customer.New Record') }}</a>
...
<script type="text/javascript">
   $(document).ready(function() {
      ...
      var maindetails_list_table = $('#maindetails_list').DataTable({
         ...
         ajax: {
            "url": 'qpurchase-bill-settlement-list',
            "type": "POST",
            "data": function(data) {
   ...
   $(document).on('click', '.bill_settlement_approve', function() {
      ...
      swal.fire({
         ...
         if (result.value) {
            $.ajax({
               type: "POST",
               url: "qpurchase-bill-settilement-approve",
               dataType: "json",
               data: {
                  _token: $('#token').val(),
                  id: id,
   ...
   $(document).on('click', '.bill_settlement_delete', function() {
      ...
      swal.fire({
         ...
         if (result.value) {
            $.ajax({
               type: "POST",
               url: "qpurchase-bill-settilement-delete",
               dataType: "json",
               data: {
                  _token: $('#token').val(),
                  id: id,
