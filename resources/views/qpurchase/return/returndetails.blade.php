                  ...
                  <div class=" pr-1 pl-1" style=" width: 100%;      background-color: #f2f3f8;   margin-top: -10px;">
                     <table class="table table-striped table-bordered table-hover" id="product_table" style="table-layout:fixed; width:100%; ">
                        ...
                        <tbody>
                           ...
                           $totalDicaountAmount = 0;
                           $toatalVatAmount = 0;
                           $grandTotal = 0;
                           ?>
                           @foreach($pi_product as $key=>$pi_product)
                           @if($pi_product->quantity>$pi_product->returned_qty)
                           <tr>
                              <td>{{$key+1}}</td>
                              <td>
                                 ...
                                 <input type="hidden" class="form-control qbuy_purchase_pi_products_id" name="qbuy_purchase_pi_products_id[]" id="qbuy_purchase_pi_products_id" data-id="{{$key+1}}" value="{{$pi_product->id}}">
                              ...
                              <td><select class="form-control kt-selectpicker" id="unit{{$key+1}}" name="unit[]" data-id="{{$key+1}}" disabled>
                                    ...
                                    @if($pi_product->unit == $data->id)
                                    <option value="{{$data->id}}" selected="">{{$data->unit_code}}</option>
                                    @endif @endforeach
                                 </select></td>
                              <td>
                                 <input type="text" class="form-control quantity " name="pquantity[]" id="pquantity{{$key+1}}" data-id="{{$key+1}}" value="{{$pi_product->quantity}}" readonly>
                                 <input type="hidden" name="inv_discount[]" id="inv_discount{{$key+1}}" value="{{$pi_product->rdiscount}}">

                              </td>
                              <td><input type="text" class="form-control quantity " name="returned_qty[]" id="returned_qty{{$key+1}}" data-id="{{$key+1}}" value="{{$pi_product->returned_qty}}" readonly></td>
                              <td><input type="text" class="form-control quantity " name="bquantity[]" id="bquantity{{$key+1}}" data-id="{{$key+1}}" value="{{$pi_product->quantity-$pi_product->returned_qty}}" readonly></td>
                              <td><input type="text" class="form-control quantity integerVal" name="quantity[]" id="quantity{{$key+1}}" data-id="{{$key+1}}" value="{{$pi_product->quantity-$pi_product->returned_qty}}"></td>
                              <td><input type="text" class="form-control rate integerVal" name="rate[]" id="rate{{$key+1}}" data-id="{{$key+1}}" value="{{$pi_product->rate}}" readonly></td>
                              ...
                              <?php
                              $curentQty = $pi_product->quantity - $pi_product->returned_qty;
                              $currentAmount = $curentQty * $pi_product->rate;
                              $currentDiscount = 0;
                              $currentDiscountAmount = 0;
                              if ($discount_type == 2) { //percentage
                                 $currentDiscount = $pi_product->rdiscount;
                                 $currentDiscountAmount = $currentAmount * ($pi_product->rdiscount / 100);
                              } else if ($discount_type == 1) { //flat
                                 $currentDiscount = ($pi_product->rdiscount / $pi_product->quantity) * $curentQty;
                                 $currentDiscountAmount = $currentDiscount;
                              }
                              $amountAfterDiscount = $currentAmount - $currentDiscountAmount;
                              $currentVatAmount = $amountAfterDiscount * ($pi_product->vat_percentage / 100);
                              $currentTotal = $amountAfterDiscount + $currentVatAmount;

                              // 
                              $totalAmountBeforeTax += $currentAmount;
                              $totalDicaountAmount += $currentDiscountAmount;
                              $toatalVatAmount += $currentVatAmount;
                              $grandTotal += $currentTotal;
                              // 
                              ?>

                              <td><input type="text" class="form-control discountamount" name="discountamount[]" id="discountamount{{$key+1}}" data-id="{{$key+1}}" value="{{$pi_product->rdiscount}}" readonly></td>
                              <td>
                                 <select class="form-control vat_percentage kt-selectpicker" data-id="{{$key+1}}" name="vat_percentage[]" id="vat_percentage{{$key+1}}" disabled>
                                    @foreach($taxgrouplist as $data)
                                    <option value="{{$data->total}}" {{($pi_product->vat_percentage == $data->total)?'Selected':''}}>{{$data->total}}</option>
                                    @endforeach
                                 </select>
                              </td>
                              <td><input type="text" class="form-control vatamount" name="vatamount[]" id="vatamount{{$key+1}}" data-id="{{$key+1}}" value="{{$pi_product->vatamount}}" readonly></td>
            ...
            <div class="row mt-5">
               ...
               <div class="col-lg-6">
                  <div class="form-group  row pr-md-3">
                     ...
                     <div class="col-md-8 input-group-sm">
                        <input type="text" class="form-control" name="totalamount" id="totalamount" readonly style=" text-align: right; font-size: 1.25rem; background-color: #f2f3f8;  height: calc(1.25em + 1rem + 2px);" value="{{$totalAmountBeforeTax}}">
                     </div>
                  </div>
               </div>
               <div class="col-lg-6">
               ...
               <div class="col-lg-6">
                  <div class="form-group  row pr-md-3">
                     ...
                     <div class="col-md-8 input-group-sm">
                        <input type="text" class="form-control discount" name="discount" id="discount" value="{{$totalDicaountAmount}}" readonly style=" text-align: right; font-size: 1.25rem; background-color: #f2f3f8;  height: calc(1.25em + 1rem + 2px);">
                     </div>
                  </div>
               </div>
               <div class="col-lg-6">
               ...
               <div class="col-lg-6">
                  <div class="form-group  row pr-md-3">
                     ...
                     <div class="col-md-8 input-group-sm">
                        <input type="text" class="form-control" name="amountafterdiscount" id="amountafterdiscount" readonly style=" text-align: right; font-size: 1.25rem; background-color: #f2f3f8;  height: calc(1.25em + 1rem + 2px);" value="{{$totalAmountBeforeTax-$totalDicaountAmount}}">
                     </div>
                  </div>
               </div>
               <div class="col-lg-6">
               ...
               <div class="col-lg-6">
                  <div class="form-group  row pr-md-3">
                     ...
                     <div class="col-md-8 input-group-sm">
                        <input type="text" class="form-control" name="totalvatamount" id="totalvatamount" readonly style=" text-align: right; font-size: 1.25rem; background-color: #f2f3f8;  height: calc(1.25em + 1rem + 2px);" value="{{$toatalVatAmount}}">
                     </div>
                  </div>
               </div>
               <div class="col-lg-6">
               ...
               <div class="col-lg-6">
                  <div class="form-group  row pr-md-3">
                     ...
                     <div class="col-md-8 input-group-sm">
                        <input type="text" class="form-control" name="grandtotalamount" id="grandtotalamount" readonly value="{{$grandTotal}}" style="background-color: #ffffff00;  border: none;  text-align: right;  font-size: 1.75rem;font-weight: bold; color: #646c9a; padding-top: 0px;">
                     </div>
                  </div>
               </div>
            </div>
   ...
   function tableForwardCalculation() {
      ...
      for (let rowId = 1; rowId <= rowcount; rowId++) {
         ...
         if (discountType == 1) { //flat discount
            var actualDiscount = getNumAsFloat($('#inv_discount' + rowId).val());
            var soQty = getNumAsFloat($('#pquantity' + rowId).val());
            var currentDiscountAsperQty = actualDiscount / soQty;
            var rtnQty = getNumAsFloat($('#quantity' + rowId).val());
            var newDiscount = currentDiscountAsperQty * rtnQty;
            $('#discountamount' + rowId).val(newDiscount);
         }
