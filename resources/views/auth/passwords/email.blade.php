...
<div class="bg-white shadow rounded w-full max-w-md mx-auto" style="z-index: 2;">
    ...
    <form method="POST" action="{{ route('password.email') }}" class="bg-white shadow rounded px-8 pt-6 pb-8 mb-4">
        @csrf
        <div class="mb-4">
            <label class="uppercase block text-gray-700 text-sm font-bold mb-2" for="emailaddress">
                Email Address
            </label>
            <input type="email" name="email" id="emailaddress" class="shadow appearance-none border @error('email') border-red-500 @enderror rounded w-full py-2 px-3 text-gray-700 mb-3 leading-tight focus:outline-none focus:shadow-outline" value="{{ old('email') }}" placeholder="email@example.com" required autocomplete="email" autofocus>
            @error('email')
