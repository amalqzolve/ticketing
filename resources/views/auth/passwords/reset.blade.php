...
<div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; z-index: -1; background:url('../images/SMS.png'); background-size: cover; background-repeat: no-repeat; filter: blur(6px); overflow: none;"></div>
...
<div class="bg-white shadow rounded w-full max-w-md mx-auto sm:mt-3" style="z-index: 2;">
    ...
    <form method="POST" action="{{route('password.update')}}" class="w-full max-w-lg px-8 mt-6 pb-8 mb-4">
        @csrf
        ...
        <div class="flex flex-wrap -mx-3 mb-6">
            <div class="w-full">
                ...
                <input type="email" name="email" id="emailaddress" class="shadow appearance-none border @error('email') border-red-500 @enderror rounded w-full py-2 px-3 text-gray-700 mb-3 leading-tight focus:outline-none focus:shadow-outline" value="{{ $email ?? old('email') }}" placeholder="email@example.com" autofocus>
                @error('email')
