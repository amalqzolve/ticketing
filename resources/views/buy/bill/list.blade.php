                           ...
                           <div class="kt-portlet__head-toolbar">
                              <div class="kt-portlet__head-wrapper">
                                 <div class="kt-portlet__head-actions">
                                     <a type="button" class="btn btn-brand btn-elevate btn-icon-sm" href="{{url('/')}}/buy_bill_settlement_add" ><i class="la la-plus"></i>{{ __('customer.New Record') }}</a> 
...
<table class="table table-striped table-hover table-checkable dataTable no-footer" id="billsettlement">
    <thead>
        <tr>
...
<tr >
   <td >{{$i}}</td>
   <td  style="cursor: pointer;"  class="viewchilden">{{ $bill->sup_name  }}</td>

    <td  style="cursor: pointer;"  class="viewchilden">{{ $bill->transactiondate  }}</td>

     <td  style="cursor: pointer;"  class="viewchilden">{{ $bill->bills  }}</td>
      <td  style="cursor: pointer;"  class="viewchilden">{{ $bill->paidamount  }}</td>
          ...
          <td><span style="overflow: visible; position: relative; width: 80px;">
            <div class="dropdown"><a data-toggle="dropdown" class="btn btn-sm btn-clean btn-icon btn-icon-md">
                     ...
                     <!--      <a href="{{url('/')}}/buy_bill_edit?id={{$bill->id}}" data-type="edit" data-target="#kt_form"><li class="kt-nav__item">
                        <span class="kt-nav__link">
...
<script type="text/javascript">
   ...
   $(document).ready(function() {
      ...
      var billsettlement_table = $('#billsettlement').DataTable({
        processing: true,
         serverSide: false,
...
$("#voucher_details_list_print").on("click", function() {
    ...
    billsettlement_table.button('.buttons-print').trigger();
});


$("#voucher_details_list_copy").on("click", function() {
    billsettlement_table.button('.buttons-copy').trigger();
});

$("#voucher_details_list_csv").on("click", function() {
    billsettlement_table.button('.buttons-csv').trigger();
});

$("#voucher_details_list_pdf").on("click", function() {
    billsettlement_table.button('.buttons-pdf').trigger();
});
