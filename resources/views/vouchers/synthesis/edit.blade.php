				...
				<div class="row" style="padding-bottom: 6px;">
					...
					<div class="col-lg-6">
						<div class="form-group row pr-md-3">
							<div class="col-md-4">
								<label>Voucher Details<span style="color: red">*</span></label>
							</div>
							<div class="col-md-8 input-group input-group-sm">
								<select class="form-control single-select kt-selectpicker" id="mrCat">
									<option value="">Select</option>
									@foreach ($MaterialCategory as $key => $value)
									<option value="{{$value->id}}" selected>{{$value->prefix}}~{{$value->voucher_name}}</option>
									@endforeach
								</select>
							</div>
						</div>
						...
						<thead class="thead-light">
							<tr>
								<th style="background-color:  #3f4aa0;
                                       color: white; white-space: nowrap;     padding: 2px 7px !important;" width="60px">Priority</th>
							...
							<td>
								...
								<div>
									<select class="form-control form-control-sm single-select unit kt-selectpicker" data-id="{{$key+1}}" name="users[]" id="user{{$key+1}}">
										...
										<option value="{{$data->id}}" {{($value->user_id==$data->id)?'selected':'' }}>{{$data->name}}</option>
										@endforeach
									</select>
								</div>
							</td>
					...
					<div class="kt-form__actions">
						<div class="row">
							...
							<div class="col-lg-6 kt-align-right">
								...
								<button type="reset" class="btn btn-secondary backHome cancel" onclick="back()">@lang('app.Cancel')</button>
							</div>
						</div>
					</div>
				</div>
...
<script src="{{url('/')}}/resources/js/vouchers/synthesis/edit.js" type="text/javascript"></script>
<script>
