...
<br/>
							<div class="kt-portlet kt-portlet--mobile">
								...
								<ul class="nav nav-tabs">
								  ...
								  <li class="nav-item">
									<a class="nav-link active" data-toggle="tab" href="#menu1">@lang('app.PDF Header')</a>
								  </li>
								<?php $header=""; ?>
								  @php foreach ($documents as $key => $value) {

								   $header =isset($value->pdfheader)? $value->pdfheader :'';
								  		} @endphp
								</ul>
								...
								<div class="tab-pane container active" id="menu1">
								   ...
								   <input type="hidden" name="fileData" value="{{$header}}" id="fileData"/>
								   <div id="choose-files">
								   <form action="/upload">
												...
												<div class="kt-form__actions">
													<div class="row">
														...
														<div class="col-lg-6 kt-align-right">
                                                            ...
                                                            <button type="reset" class="btn btn-primary btn-icon-sm" id="pdfheader_submit">
    ...
    allowedFileTypes: ['image/*'],
		...
		onBeforeUpload: (files) => {
			...
			Object.keys(files).forEach(fileID => {
					fileData.push('pdfheader/' + files[fileID].name)
				})
				//return updatedFiles
...
$(document).on('click', '#pdfheader_submit', function(e) {
	e.preventDefault();

	$.ajax({
		type: "POST",
		url: "pdfheader_submit",
		dataType: "json",
		data: {
			_token: $('#token').val(),
			id: $('#id').val(),
		...
		success: function(data) {
			...
			$('#pdfheader_submit').removeClass('kt-spinner');
			$('#pdfheader_submit').prop("disabled", false);
			toastr.success('Sucessfully added');

			// swal.fire("Done", "Submission Sucessfully", "success");
			location.reload();
			window.location.href = "branchsettings_header";
