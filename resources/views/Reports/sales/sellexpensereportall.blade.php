									...
									<div class="kt-portlet__head-toolbar">
										<div class="kt-portlet__head-wrapper">
											<div class="kt-portlet__head-actions">
												...
												<a href="{{url('/')}}/sell_expense_reportpdf?fromdate={{$fromdate}}&&todate={{$todate}}"  target="_blank" class="btn btn-brand btn-elevate btn-icon-sm">
													PDF
												</a>&nbsp;
...
<tr>
<td>{{$key+1}}</td>
<td>{{$details->main_voucher_id}}</td>
<td>{{$details->entrydate}}</td>
<td >Expense</td>
<td style="text-align:right;">{{$details->cust_name}}</td>
<td>{{$details->head_names}}</td>
<td>{{$details->bill_id}}</td>
<td style="text-align:right;">{{number_format($details->grandtotalamount,2,'.',',')}}</td>
<?php $total += $details->grandtotalamount;?>
