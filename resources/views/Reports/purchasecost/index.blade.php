...
<form method="POST" action="{{ route('purchasecostsubmit') }}">
	@csrf
<div class="row">
								...
								<select class="form-control kt-selectpicker" name="purchaseno" id="purchaseno">
								<option>Select</option> 
									@foreach($purchaseno as $purchasenos)
									<option value="{{$purchasenos->id}}">{{$purchasenos->id}}</option>
									@endforeach
								</select>              
										</div>
										</div>
...
<div class="col-lg-2">
	  <button type="submit" id="purchasecostsubmit" class="btn btn-primary" style="float:right;">Submit</button>
</div>
</div>
</form>
