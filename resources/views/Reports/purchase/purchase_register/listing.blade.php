...
<div class="kt-container  kt-container--fluid  kt-grid__item kt-grid__item--fluid">
	...
	<div class="kt-portlet kt-portlet--mobile">
		...
		<div class="kt-portlet__body">
			...
			<!--begin: Datatable -->
			<form method="GET" id="reportForm" action="{{ route('purchase_register') }}" autocomplete="off">
				<div class="row">
					<div class="col-lg-5">
					...
					<div class="col-lg-2">
						...
						@if (!empty($tableOn))
						<br>
						<a style="margin-top: 10px;" href="{{url('/')}}/purchase_registerpdf?fromdate={{$fromdate}}&&todate={{$todate}}" target="_blank" class="btn btn-brand btn-elevate btn-icon-sm"> View PDF</a>
						@endif
					</div>
					<br><br>
					<?php if (!empty($tableOn)) { ?>
						<table class="table table-striped table-hover table-checkable dataTable no-footer" id="dt">
							...
							<tr>
								...
								<!-- <td>{{$details['advance_amt']}}</td> -->
								<td>{{$details['paid_by_cash']}}</td>
								<td>{{$details['paid_by_card']}}</td>
								<td>{{$details['paid_by_bank']}}</td>
								<!-- <td>{{$details['by_billsettilement']}}</td> -->
								<td>{{$details['paid_by_cash']+$details['paid_by_card']+$details['paid_by_bank']}}</td>
								<td>{{$details['grandtotalamount']-($details['paid_by_cash']+$details['paid_by_card']+$details['paid_by_bank'])}}</td>
							</tr>
							<?php
							...
							<tr>
								...
								<!-- <td> <b><i> {{$advance_amt}}</i></b></td> -->
								<td> <b><i> {{$paid_by_cash}}</i></b></td>
								<td> <b><i> {{$paid_by_card}}</i></b></td>
								<td> <b><i> {{$paid_by_bank}}</i></b></td>
								<!-- <td> <b><i> {{$by_billsettilement}}</i></b></td> -->
...
<script>
	...
	$('.purchase_register').addClass('kt-menu__item--active');
</script>
