...
var tblProformaInvoice = $('#tblProformaInvoice').DataTable({
    processing: true,
    serverSide: true,
    ...
    columns: [
        ...
        {
            data: 'car_in_out_id', name: 'car_in_out_id', render: function (data, type, row) {
                return 'TRIP -' + row.car_in_out_id;
            }
        },
        {
        ...
        { data: 'grandtotalamount', name: 'grandtotalamount' },
        {
            ...
            render: function (data, type, row) {
                if (row.status == 0)
                    return '<span style="color: black">Not Invoiced</span>';
...
$(document).on('click', '.delete', function () {
    ...
    swal.fire({
        title: "Are you sure?",
        text: "Do you want trash this Proforma Invoice",
        type: "warning",
        ...
        if (result.value) {
            $.ajax({
                type: "POST",
                url: "../trip-proforma-invoice-delete",
                dataType: "json",
                data: {
                    _token: $('#token').val(),
                    id: noteId
                ...
                success: function (data) {
                    if (data.status == 1) {
                        toastr.success('Proforma Invoice Trashed successfuly');
                        location.reload();
                    } else
