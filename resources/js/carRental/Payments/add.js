...
$(document).on('click', '#advance_submit', function (e) {
    e.preventDefault();
    var error = 0;
    if ($('#salesman').val() == "") {
    ...
    if (!error) {
        $('#advance_submit').addClass('kt-spinner');
        ...
        $.ajax({
            type: "POST",
            url: "../trip-advance-submit",
            dataType: "json",
            data: $('#dataForm').serialize() + "&_token=" + $('#token').val(),
            success: function (data) {
                if (data.status == 1) {
                    $('#advance_submit').removeClass('kt-spinner');
                    $('#advance_submit').prop("disabled", false);
                    toastr.success('Adwance ' + sucess_msg + ' Successfuly');
                    window.location.href = "../trip-advance/" + data.key;
                }
                else {
                    $('#advance_submit').removeClass('kt-spinner');
                    $('#advance_submit').prop("disabled", false);
