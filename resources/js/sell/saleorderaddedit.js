$(document).on('click', '#quotation_submit', function (e) {
    ...
    if ($('#newcustomer').val() == 2) {
        ...
        if ($('#account_ledger').val() == '') {
            ...
            toastr.warning("Please Select Ledger Account!");
            return false;
        }
        else
    ...
    function saveDetails(result) {
        ...
        var status = (result.isConfirmed) ? "Confirm" : "Draft";
        $('#quotation_submit').addClass('kt-spinner');
        $('#quotation_submit').prop("disabled", true);
        ...
        $.ajax({
            ...
            data: {
                ...
                account_ledger: $('#account_ledger').val(),
                dateofsupply: $('#dateofsupply').val(),
                method: $('#method').val(),
                status: status
            },
            success: function (data) {
                toastr.success('New Sales Order ' + sucess_msg + ' successfuly');
...
$(document).on('click', '.quotation_approve', function () {
    ...
    swal.fire({
        title: "Are you sure?",
        text: "Do you want Approve this Quotation",
        type: "warning",
        showCancelButton: true,
        confirmButtonClass: "btn-danger",
        confirmButtonText: "Yes",
        ...
        if (result.value) {
            window.location = "Quotation-Approve?id=" + id;
...
$(document).on('click', '#quotation_approve', function (e) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            window.location.href = "sell_saleorder_list";
            toastr.success('Sales Order Generated Successfuly');
