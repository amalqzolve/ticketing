...
$(document).on('click', '#invoice_edit_sell', function (e) {
    ...
    var productrowid = [];
    $("input[name^='productrowid[]']").each(function (input) {
        productrowid.push($(this).val());
    });
    ...
    swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            updateNormalType(result)
        } else if (result.isDenied) {
            // Swal.fire('Changes are not saved', '', 'info')
            updateNormalType(result)
        } else {
            swal.fire("Cancelled", "", "error");
            $('#invoice_edit_sell').removeClass('kt-spinner');
            $('#invoice_edit_sell').prop("disabled", false);
        }
    });

    function updateNormalType(result) {
        var status = (result.isConfirmed) ? "Approved" : "Draft";
        $('#invoice_edit_sell').addClass('kt-spinner');
        $('#invoice_edit_sell').prop("disabled", true);
...
$(document).on('click', '#invoice_edit_sell_byquote', function (e) {
    e.preventDefault();
    ...
    var invoiced = [];
    $("input[name^='invoiced[]']").each(function (input) {
        invoiced.push($(this).val());
    });
    ...
    function updateInvoice(result) {
        ...
        $.ajax({
            type: "POST",
            url: "invoiceupdate_sell_byquote",
            dataType: "json",
            data: {
                _token: $('#token').val(),
                id: $('#invoice_id').val(),
