...
$('.addmorepayments').click(function () {
    ...
    $trNew.find('select[name="depositaccount[]"]').val('');
...
$("body").on("click", ".costremove", function (event) {
    ...
    if (tblRowCount > 2) {
        ...
        $('select[name="depositaccount[]"]').val('');
...
function chekPayFromAdvance() {
    var existBalance = parseFloat(($('#creditBalance').val() != '') ? $('#creditBalance').val() : 0);//$('#creditBalance').val(); //
