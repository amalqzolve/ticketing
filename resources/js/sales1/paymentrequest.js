var paymentrequestdetails_list_table = $('#paymentrequestdetails_list').DataTable({
      processing: true,
      serverSide: true,
      ...
      columns: [
          ...
          
          {
              ...
              render: function(data, type, row) {
                var j='';
                     j='<a href="paymentrequest_edit?id=' + row.id +'" target="" data-type="edit" data-target="#kt_form" ><li class="kt-nav__item">\
                        <span class="kt-nav__link">\
                        <i class="kt-nav__link-icon flaticon2-file-1"></i>\
                        <span class="kt-nav__link-text" data-id="' + row.id + '" >Edit</span>\
                        </span></li></a><a href="paymentrequest_pdf?id=' + row.id +'" target="_blank" data-type="edit" data-target="#kt_form" ><li class="kt-nav__item">\
                        <span class="kt-nav__link">\
                        <i class="kt-nav__link-icon flaticon2-file-1"></i>\
                        <span class="kt-nav__link-text" data-id="' + row.id + '" >PDF</span>\
                        </span></li></a><a href="paymentrequest_pdf_letter?id=' + row.id +'" target="_blank" data-type="edit" data-target="#kt_form" ><li class="kt-nav__item">\
                        <span class="kt-nav__link">\
                        <i class="kt-nav__link-icon flaticon2-file-1"></i>\
                        <span class="kt-nav__link-text" data-id="' + row.id + '" >PDF Letter Head</span>\
                        </span></li></a><li class="kt-nav__item">\
                        <span class="kt-nav__link">\
                        <i class="kt-nav__link-icon flaticon2-trash"></i>\
                        <span class="kt-nav__link-text paymentrequest_delete" id=' + row.id + ' data-id=' + row.id + '>Delete</span></span></li>';
...
$(document).on('click', '#payment_request_submit', function(e) {
    e.preventDefault();
   ...
   var vat_percentage = [];
    ...
    $.ajax({
        ...
        success: function(data) {
             ...
             $('#payment_request_submit').removeClass('kt-spinner');
             $('#payment_request_submit').prop("disabled", false);
             location.reload();
              window.location.href = "paymentrequest";
             toastr.success('Payment Request'+sucess_msg+' successfuly');
 ...
 $(document).on('click', '.paymentrequest_delete', function() {
        var id = $(this).attr('id');
        swal.fire({
            title: "Are you sure?",
