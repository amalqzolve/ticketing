...
$('.epr-transfer-request').addClass('kt-menu__item--active');
...

        {
            ...
            render: function (data, type, row) {
                if (row.transfer_status == 0)
                    return '<span style="color: black">Not Transferred</span>';
                if (row.transfer_status == 1)
                    return '<span style="color: blue">Partially Transferred</span>';
                if (row.transfer_status == 2)
                    return '<span style="color: orange">Fully Transferred</span>';
            ...
            render: function (data, type, row) {
                ...
                if (row.transfer_status != 2) {
                    j = '<a href="stock-transfer-pdf?id=' + row.id + '" target="_blank" data-type="edit" data-target="#kt_form" ><li class="kt-nav__item">\
                        ...
                        <a href="add-transfer-stock?id=' + row.id + '" data-type="edit" data-target="#kt_form"><li class="kt-nav__item">\
                        <span class="kt-nav__link">\
                        <i class="kt-nav__link-icon flaticon2-telegram-logo"></i>\
                        <span class="kt-nav__link-text" data-id="' + row.id + '" >Generate Delivery Order</span>\
                        </span></li></a>';
                }
...
var completed = $('#completed').DataTable({
    processing: true,
    serverSide: true,
    scrollX: true,
    pagingType: "full_numbers",
    ...
    ajax: {
        "url": 'epr-transfer-request-completed',
        "type": "POST",
        "data": function (data) {
            data._token = $('#token').val()
        }
...
$(document).on('click', '.send_req', function () {
    ...
    swal.fire({
        title: "Are you sure?",
        text: "Do you want Send this Stock Transfer For Approval",
        type: "warning",
