$('.tenderParticipation').addClass('kt-menu__item--active');
var pendingVouchers = $('#draft').DataTable({
   ...
   lengthMenu: [
    ...
    columns: [
        ...
        {
            data: 'participation_status', name: 'participation_status',
            render: function (data, type, row) {
                if (row.participation_status == 0)
                    return "<span style='color: blue'>Tender Approved</span>";
                if (row.participation_status == 1)
                    return '<span style="color: green">Can Participate</span>';
            }
        },
        {
            data: 'action',
            ...
            render: function (data, type, row) {
                ...
                if (row.participation_status == 0) {
                    ...
                    <a data-type="approved" data-target="#kt_form"><li class="kt-nav__item participationAction" id=' + row.id + '>\
                    <span class="kt-nav__link">\
                    <i class="kt-nav__link-icon flaticon2-check-mark"></i>\
                    <span class="kt-nav__link-text" data-id="' + row.id + '" id=' + row.id + '>Participate</span>\
                    </span></li></a>\
...
$(document).on('click', '.participationAction', function () {
    var id = $(this).attr('id');
    swal.fire({
        title: "Are you sure?",
        text: "Do you want participate this tender",
        type: "warning",
        ...
        if (result.value) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.status == 1) {
                        loaderClose();
                        toastr.success('Tender status marked successfuly');
                        window.location.href = "participation-list";
                    } else {
