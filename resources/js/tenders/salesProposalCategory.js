...
$('.sales-proposal-category').addClass('kt-menu__item--active');
    ...
    columns: [
        ...
        },
        {
          ...
          <div class="dropdown"><a data-toggle="dropdown" class="btn btn-sm btn-clean btn-icon btn-icon-md">\
                      ...
                      <ul class="kt-nav">\
                      <a href="sales-proposal-category-edit?id=' + row.id + '" data-type="edit" data-target="#product_category"><li class="kt-nav__item">\
...
var tax_trash_list_table = $('#tax_trash_list').DataTable({
   ...
   lengthMenu: [
    ...
    ajax: {
        "url": 'sales-proposal-settingstaxTrash',
        "type": "POST",
        "data": function (data) {
            data._token = $('#token').val()
        }
...
$(document).on('click', '#tax_submit', function (e) {
    ...
    $.ajax({
        type: "POST",
        url: "sales-proposal-category-submit",
        dataType: "json",
        data: {
            _token: $('#token').val(),
            info_id: $('#id').val(),
        ...
        success: function (data) {
            if (data == false) {
                ...
                $('#tax_submit').removeClass('kt-spinner');
                $('#tax_submit').prop("disabled", false);
                window.location.href = "sales-proposal-category";
...
$(document).on('click', '.mrCatdelete', function () {
    ...
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                type: "POST",
                url: 'sales-proposal-category-delete',
                dataType: "json",
                data: {
                    _token: $('#token').val(),
                    id: id
...
$(document).on('click', '.taxrestore', function () {
    ...
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                type: "POST",
                url: 'sales-proposal-settingsrestore-tax',
                data: {
                    _token: $('#token').val(),
                    id: id
                },
...
$(document).on('click', '.taxtrashdelete', function () {
    ...
    swal.fire({
        ...
        if (result.value) {
            ...
            $.ajax({
                type: "POST",
                url: 'sales-proposal-settingstrashdelete-tax',
                data: {
                    _token: $('#token').val(),
                    id: id
                },
