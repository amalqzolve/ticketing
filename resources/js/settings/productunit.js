			...
			columns: [
					...
					
					{
						...
						<div class="dropdown"><a data-toggle="dropdown" class="btn btn-sm btn-clean btn-icon btn-icon-md">\
												...
												<ul class="kt-nav">\
												<a href="settingsedit_productunit?id=' + row.id + '" data-type="edit" data-target="#kt_form"><li class="kt-nav__item">\
	   ...
	   return false;
				...
				$.ajax({
							...
							if(data == 'false')
					{
						...
						toastr.success('Product unit already exist');
					}
					else
					{
										$('#Productunitsubmit').removeClass('kt-spinner');
										...
										window.location.href = "settingsUnitList";
									}
						},
						...
						if (result.value) {
								...
								$.ajax({
										...
										success: function(data) {
											if(data == '1')
											{
												swal.fire("Not Deleted!", "Unit is already used.", "success");
												location.reload();
											}
...
*/
		$(document).on('click', '.Restoreunitinventory', function () {
			var id = $(this).attr('id');
				$.ajax({
							type: "POST",
							url : 'settingsrestoreinventoryunit',
							data: {
											_token : $('#token').val(),
											id     : id
										},
			...
			$(document).on('click', '.Productunittrashdelete', function() {
				...
				swal.fire({
						...
						if (result.value) {
								...
								$.ajax({ type: "POST", url: 'settingsDeleteTrashProdctunits',
								data: {
												_token: $('#token').val(),
												id: id
										},
