...
$(document).on("click", ".vcmt", function () {
    ...
    localStorage.setItem('currentCustomerChat', currentComment);
...
uppyProject.use(Uppy.XHRUpload, {
    endpoint: '../project-customer-feedback-file-upload',
    fieldName: 'filenames[]',
    headers: {
...
$(document).on('click', '#btnPostMainComment', function (e) {
    ...
    if (!error) {
        ...
        $.ajax({
            type: "POST",
            url: "../project-customer-feedback-submit",
            dataType: "json",
...
$(document).on('click', '.commentTrash', function (e) {
    ...
    swal.fire({
        ...
        if (result.value) {
            $.ajax({
                type: "POST",
                url: '../project-customer-feedback-delete',
                data: {
                    _token: $('#token').val(),
                    id: id
                },
...
$(document).on('click', '.subCommentTrash', function (e) {
    ...
    swal.fire({
        ...
        if (result.value) {
            $.ajax({
                type: "POST",
                url: '../project-sub-customer-feedback-delete',
                data: {
                    _token: $('#token').val(),
                    id: id
                },
...
$(document).on('click', '.btnPostSubComment', function (e) {
    ...
    if (!error) {
        ...
        $.ajax({
            type: "POST",
            url: "../project-sub-customer-feedback-submit",
            dataType: "json",
            data: formData,
            processData: false,
            contentType: false,
...
$(document).ready(function () {
    var chatId = localStorage.getItem('currentCustomerChat');
    if (chatId != '')
        $('#btn_' + chatId).click();
    localStorage.setItem('currentCustomerChat', '');
});