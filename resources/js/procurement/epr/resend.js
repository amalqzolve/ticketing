...
$(document).on('click', '#epr_resend', function (e) {
    e.preventDefault();
    var error = 0;
    var procuctEnter = 0;
    var productname = [];
    ...
    if (error == 0) {
        $('#epr_resend').addClass('kt-spinner');
        $('#epr_resend').prop("disabled", true);
        loaderShow();
        $.ajax({
            type: "POST",
            url: "material-request-resend",
            dataType: "json",
            data: {
                _token: $('#token').val(),
                id: id,
                version: $('#materialRequestVersion').val(),
            ...
            success: function (data) {
                loaderClose();
                if (data.status = 1) {
                    $('#epr_resend').removeClass('kt-spinner');
                    $('#epr_resend').prop("disabled", false);
                    toastr.success('EPR Re-send successfuly');
