...
var draftTbl = $('#draftTbl').DataTable({
    ...
    columns: [
        ...
        { data: 'grn_created_date', name: 'grn_created_date' },
        {
            data: 'po_id',
            name: 'po_id',
            render: function (data, type, row) {
        ...
        {
            data: 'stock_in_status',
            name: 'stock_in_status',
            render: function (data, type, row) {
                var out;
                if (row.stock_in_status == 0)
                    out = 'Not Generated';
                else if (row.stock_in_status == 1)
                    out = 'Partially Generated';
                else if (row.stock_in_status == 2)
                    out = 'Generated';
                return out;
        ...
        },
        {
            ...
            render: function (data, type, row) {
                var j = '';
                if (row.status == 1) {
                    j = '<a href="epr-po-grn-pdf?id=' + row.id + '" target="_blank" data-type="edit" data-target="#kt_form" ><li class="kt-nav__item">\
                        <span class="kt-nav__link">\
                        <i class="kt-nav__link-icon flaticon2-file-1"></i>\
                        <span class="kt-nav__link-text" data-id="' + row.id + '" >PDF</span>\
                        </span></li></a>\
                        <a href="suggestion-send?id=' + row.id + '&type=GRN" target="_blank" data-type="edit" data-target="#kt_form" ><li class="kt-nav__item">\
                        <span class="kt-nav__link">\
                        <i class="kt-nav__link-icon flaticon2-file-1"></i>\
                        ...
                        <a href="epr-po-grn-edit-view?id=' + row.id + '" data-type="edit" data-target="#kt_form"><li class="kt-nav__item">\
                        <span class="kt-nav__link">\
                ...
                if (row.status == 3) {
                    ...
                    <a href="epr-po-grn-resubmit?id=' + row.id + '" data-type="edit" data-target="#kt_form"><li class="kt-nav__item">\
                    <span class="kt-nav__link">\
                    <i class="kt-nav__link-icon flaticon2-reload-1"></i>\
                ...
                if (row.status == 5 || row.status == 6) {
                    ...
                    <a href="epr-po-invoice-booking?id=' + row.po_id + '" data-type="edit" data-target="#kt_form"><li class="kt-nav__item">\
                    <span class="kt-nav__link">\
                    <i class="kt-nav__link-icon flaticon2-paperplane"></i>\
                    <span class="kt-nav__link-text" data-id="' + row.po_id + '" >Convert To Invoice</span>\
                    </span></li></a>\
                    <a href="grn-send-to-ware-house?id=' + row.id + '" data-type="edit" data-target="#kt_form"><li class="kt-nav__item">\
                    <span class="kt-nav__link">\
                    <i class="kt-nav__link-icon flaticon2-reload-1"></i>\
                    <span class="kt-nav__link-text" data-id="' + row.id + '" >Material Transfer</span>\
                    </span></li></a>';
                }
                if (row.status == 0) {
                    j = '<a href="epr-po-grn-pdf?id=' + row.id + '" data-type="edit" data-target="#kt_form"><li class="kt-nav__item">\
                        <span class="kt-nav__link">\
                        <i class="kt-nav__link-icon flaticon2-telegram-logo"></i>\
...
var apptovedTbl = $('#apptovedTbl').DataTable({
    processing: true,
    serverSide: true,
    pagingType: "full_numbers",
    dom: 'Blfrtip',
...
$(document).on('click', '.sendForApproval', function () {
    ...
    swal.fire({
        title: "Are you sure?",
        text: "Do you want Send this GRN For Approval",
        type: "warning",
        ...
        if (result.value) {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data.status == 1) {
                        loaderClose();
                        toastr.success('GRN Send for Approval successfuly');
...
$(document).on('click', '#addRec', function () {
    if ($('#po').val() == '') {
        toastr.warning("Please Select A Po for create GRN !");
        $('#po').next().find('.select2-selection').addClass('select-dropdown-error');
    } else
        window.location.href = "epr-po-grn?id=" + $('#po').val();
});
...
$("#export-button-print").on("click", function () {
    ...
    if (tbl == 1)
        ...
        apptovedTbl.button('.buttons-print').trigger();
    else if (tbl == 4)
...
$("#export-button-copy").on("click", function () {
    ...
    if (tbl == 1)
        ...
        apptovedTbl.button('.buttons-copy').trigger();
    else if (tbl == 4)
...
$("#export-button-csv").on("click", function () {
    ...
    if (tbl == 1)
        ...
        apptovedTbl.button('.buttons-csv').trigger();
    else if (tbl == 4)
...
$("#export-button-pdf").on("click", function () {
    ...
    if (tbl == 1)
        ...
        apptovedTbl.button('.buttons-pdf').trigger();
    else if (tbl == 4)
