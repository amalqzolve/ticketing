$('.stockIn').addClass('kt-menu__item--active');
var boqListTable = $('#draftTbl').DataTable({
    processing: true,
    serverSide: true,
    ...
    columns: [
        ...
        {
            data: 'warehouse_project', name: 'warehouse_project', render: function (data, type, row) {
                var curData = row.warehouse_project;
                if (curData != null)
                    return curData.length > 30 ? curData.substr(0, 30) + '…' : curData;
                else
                    return '-';
...
var sendTbl = $('#sendTbl').DataTable({
    ...
    ajax: {
        "url": 'epr-po-grn-stock-in-list-send',
        "type": "POST",
        "data": function (data) {
            data._token = $('#token').val()
        }
...
var approvedTbl = $('#approvedTbl').DataTable({
    ...
    ajax: {
        "url": 'epr-po-grn-stock-in-list-approved',
        "type": "POST",
        "data": function (data) {
            data._token = $('#token').val()
        }
...
var rejectedTbl = $('#rejectedTbl').DataTable({
    ...
    ajax: {
        "url": 'epr-po-grn-stock-in-list-rejected',
        "type": "POST",
        "data": function (data) {
            data._token = $('#token').val()
        }
