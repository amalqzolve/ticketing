...
var vanproduct_list_table = $('.productdetails_list').DataTable({
    processing: true,
    serverSide: false,
    bPaginate: false,
    dom: 'Blfrtip',
    ...
    $(document).ready(function() {
     ...
     $('#productdetails_list tbody').on( 'click', 'tr', function () {
        $(this).toggleClass('selected');
         ...
         $('#selected_items').val(vanproduct_list_table.rows('.selected').data().length);

var versement_each = 0;
selectArr= new Array();
 var ids = $.map(vanproduct_list_table.rows('.selected').data(), function (item) {
...
vanproduct_list_table .rows( '.selected' ).nodes().to$() .removeClass( 'selected' );
$('#selected_amount').val('');
$('#selected_items').val('');
...
$("#productdetails_list_print").on("click", function() {
    ...
    vanproduct_list_table.button('.buttons-print').trigger();
});


$("#productdetails_list_copy").on("click", function() {
    vanproduct_list_table.button('.buttons-copy').trigger();
});

$("#productdetails_list_csv").on("click", function() {
    vanproduct_list_table.button('.buttons-csv').trigger();
});

$("#productdetails_list_pdf").on("click", function() {
    vanproduct_list_table.button('.buttons-pdf').trigger();
});
