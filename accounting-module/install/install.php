...
if (is_file($installFile)) { 
  ...
  if($_POST){
    ...
    if ($core->write_config($domain, $enckey) == false) {
      ...
      echo "<div class='alert alert-success'><i class='icon-ok'></i> Config details written to the config files.</div>"; 
    }
  ...
  if($finished) {
    ...
    <div class="alert alert-info"><i class='icon-info-sign'></i> You can login now using the following credential:<br /><br />
      ...
      <div class="bottom">
        <a href="<?php echo "http://".$_SERVER["SERVER_NAME"].substr($_SERVER["REQUEST_URI"], 0, -24); ?>login/" class="btn btn-primary">Go to Login</a>
